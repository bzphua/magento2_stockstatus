...
class AddQuantityAvailableFieldToCollection implements \Magento\Ui\DataProvider\AddFieldToCollectionInterface
{
    public function addField(\Magento\Framework\Data\Collection $collection, $field, $alias = null)
    {
		...
		$collection->getSelect()->columns(
                array(
                    'quantity_available' => new \Zend_Db_Expr('(SELECT at_qty.qty-SUM(soi.qty_ordered) FROM sales_order_item soi INNER JOIN sales_order so ON soi.order_id=so.entity_id WHERE soi.product_id=e.entity_id GROUP BY soi.product_id)')));
